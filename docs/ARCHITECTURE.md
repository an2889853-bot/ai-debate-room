...
- `Config.web_search`(🌐 웹 검색 허용) / `Config.tools`(🛠 파일·명령 허용) / `Config.workspace`. 엔진 `Config`·UI `DEFAULT_SETTINGS`·콘솔 모두 **기본 꺼짐**(2026-09-18 공유판 기준 — 처음 받은 사람이 모르고 명령 실행을 켜지 않도록). 사이드바에서 켜면 `ui_settings.json`에 저장돼 유지, 콘솔은 `--web`, `--tools`.
...
- **경계와 위험**: 허용 목록(`TOOL_ALLOW_CMDS` = python·pytest·pip·git·ls·dir·cat·type·echo·mkdir) 안이라도 `python`은 임의 코드 실행이다. 폴더 격리·허용 목록·타임아웃·⏹ 중단은 실수 방지지 보안 경계가 아니고, 웹이 열리면 데이터가 밖으로 나갈 수 있다. 공유판(2026-09-18)부터 두 토글 모두 기본 꺼짐이며 도움말에 경고가 있다 — 최신 사실·코드 실행이 필요한 질문에만 켜고, 추론·설계 검토처럼 근거가 이미 안에 있는 질문은 끈 채 두는 편이 낫다(6절, DECISIONS 참고).
- **Codex 샌드박스와 파이썬**: `workspace-write` 샌드박스는 workspace 밖 실행 파일을 막아 venv 파이썬(uv 트램폴린 → uv 설치 폴더)이 "did not find executable"로 실패했다. `CODEX_ADD_DIRS`(= `sys.prefix`, `sys.base_prefix` 중 존재하는 것)를 `--add-dir`로 열어 해결(실기 2026-09-17: `python hello2.py → hi 43`). 쓰기 가능 폴더가 되므로 모델 명령이 venv·기반 파이썬을 건드릴 수 있다는 위험은 감수한 것 — 도구를 켠 라운드에서만.
- **도구 호출 권고 상한**: `Config.tool_budget`(기본 8, UI "단계당 도구 호출 권고 상한", 콘솔 `--tool-budget`), 평가자는 `eval_tool_budget`(기본 5, UI 값과 5 중 작은 쪽). 이 CLI 버전엔 강제 옵션(`--max-turns`)이 없어 규칙 문구로만 전달한다(`tool_rules(..., budget)`: "N회 이내, 핵심 사실부터"). 웹 규칙에 "조회 실패한 URL은 재시도하지 말고 다른 출처·검색 요약을 쓰되 실패를 밝혀라", 평가자 지시문에 "핵심 주장 5개 이내에 집중" 추가 — 실측(평가자 12회 조회, 145초, $1.14)에 대한 대응.
