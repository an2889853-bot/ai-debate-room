...
**결과**: 사용자가 GitHub Desktop(winget 설치, 바탕화면 바로가기)으로 로그인 → Add local repository → Publish. 공개 저장소 https://github.com/an2889853-bot/ai-debate-room (계정 `an2889853-bot`).
**원클릭 설치(같은 날)**: "PowerShell에 한 줄씩 붙여넣는 건 필수냐"는 질문 → 설치 자체(두 CLI·구독 로그인·파이썬)는 피할 수 없지만 붙여넣기는 없앨 수 있어 `setup.cmd` → `tools\setup.ps1`(`-ExecutionPolicy Bypass`로 실행 정책 우회, UTF-8 BOM으로 한글 메시지)을 추가. 이미 된 단계는 건너뛰고, 로그인은 상태 확인(`claude auth status --json`, `codex login status`) 후 안 된 쪽만 창을 띄움 — `codex login --device-auth`가 아닌 일반 `codex login`이며 로그인돼 있으면 부르지 않는다. 실기로는 못 돌려 봄(이 PC는 전부 설치돼 있어 "이미 있음" 경로만 타게 됨) → 문법 검사만. 받는 사람 첫 실행에서 확인 필요.
**같은 날 실기 3회로 잡은 버그**: ① `Run` 함수가 외부 명령 출력과 `return $LASTEXITCODE`를 한 배열로 돌려줘 `$rc -ne 0` 판정이 틀림(점검이 통과인데 "실패") → 출력은 `Write-Host`로 찍고 종료 코드만 반환. ② `cmd /c "$cmdline"`로 감싸면 경로 따옴표를 PowerShell이 `\"`로 넘겨 깨짐 → cmd 없이 `& $exe @argv 2>&1 | ForEach-Object { "" + $_ }`(빨간 글씨 없이 출력) / `*> $null`(종료 코드만). ③ Codex 실행 파일 탐색이 `codex*.exe` 첫 번째인 보조 프로그램 `codex-command-runner.exe`를 집어 `login status`가 "no pipe-in provided"로 실패하고 로그인 창을 또 열었음 → 본체 `codex-x86_64-pc-windows-msvc.exe`를 먼저 찾고 `runner` 제외. 사용자가 본 "빨간 글씨"는 ①·③ 때문. 또 `[Console]::OutputEncoding=UTF8`로 ✅ 출력 깨짐 방지, winget 없음 안내(https://aka.ms/getwinget), 사용자·시스템 PATH 재조립(`RefreshPath`), 문제 개수 집계. 최종 실기: 오류 레코드 0, 로그인 재요청 0, 점검 통과. `docs/GUIDE.md`(처음 받는 사람용, 막힐 때 표) 추가. 테스트 격리 수정(테스트 중 `ui_settings.json`을 빈 설정으로 — 소유자의 "도구 켜짐" 설정이 기본값 테스트에 섞여 2개가 깨졌던 것)도 함께 올라감.

## 2026-09-18 — Claude 모델 자동 대체 (첫 외부 사용자 실기)

첫 외부 사용자 PC에서 설치는 전부 됐는데 마지막 점검이 `❌ claude … api_error_status 429, credits_required, "Fable 5.1 requires usage credits"`로 실패 — 그 구독 플랜엔 Fable이 포함되지 않았다. 사용자 지적: "앱에서 다른 모델을 고를 수 있는데 왜 설치 단계가 막힌 것처럼 보이냐". 맞는 말 — 모델 하나가 토론을 멈추면 안 된다.
**결정**: `call_claude`를 `_call_claude_once`로 감싸 `CreditsRequired`(subtype은 success인데 `is_error`+`api_error_code=credits_required`)면 `CLAUDE_FALLBACK`(opus → sonnet)으로 재시도, `meta.resolve_note`에 남겨 UI 캡션과 `--check`에 표시. 다른 오류는 대체하지 않는다(원인을 숨기지 않기 위해). 설치 점검 실패 문구를 "설치는 끝났지만…"으로 바꾸고 가이드 표에 항목 추가.
**교훈**: 첫 외부 사용자에게서 나온 문제 4건(패키지 설치 중 멈춘 듯 보임, 로그인 후 창 닫음, Codex 보조 exe 오탐, 모델 크레딧) 전부 "내 PC에서는 재현 안 되는" 종류였다. 가이드와 스크립트는 남의 PC에서 굴러야 완성이다.
