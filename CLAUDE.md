...
- 코드 수정 → `pytest`(109개, 약 10초) → 커밋. 테스트 대역(`conftest.FakeCLI`, `FakeStages`)은 실제 함수 시그니처를 따라가야 한다 — 인자를 추가하면 대역도 같이. 결정·사고는 `docs/DECISIONS.md` 맨 아래에 날짜와 함께 추가하고, 동작이 바뀌면 `docs/ARCHITECTURE.md`를 같이 고친다. 이 파일엔 상태·규칙만 갱신한다.
