...
ai-debate-room\
  ...
  tests\                  conftest(픽스처·가짜 단계·가짜 CLI), test_plan(엔진), test_contract(반영 계약), test_evidence(코드 검사), test_eval(독립 평가), test_usage(관측), test_compact(압축), test_app(AppTest 종단)
