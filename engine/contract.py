...
def tool_rules(tools: bool = False, web: bool = False, web_reuse: bool = False, budget: int = 0) -> str:
    ...
    lines = []
    if tools:
        lines.append("- 현재 작업 폴더 안에서 파일 읽기/쓰기와 허용된 명령(python, pytest, pip, git, ls 등)을 실행할 수 있습니다. "
                     "허용 목록 밖 명령은 거부됩니다. 실행한 명령과 결과는 프로그램이 '[프로그램 기록 ...]' 블록으로 기록에 남기므로 "
                     "결과를 꾸미지 말고 그대로 인용하십시오. 주장하기 전에 실행으로 확인할 수 있으면 확인하십시오.\n")
    else:
        lines.append("- 파일 읽기·명령 실행은 허용되지 않습니다.\n")
    if web:
        lines.append("- 웹 검색·페이지 읽기가 허용됩니다. 최신 사실·수치·뉴스는 검색으로 확인하고, 검색으로 얻은 사실엔 출처 URL과 "
                     "확인 시각을 적으십시오. 검색 기록도 프로그램이 기록에 남깁니다. 페이지 조회에 실패한 URL은 다시 시도하지 말고 "
                     "다른 출처나 검색 결과 요약을 쓰되 실패했음을 밝히십시오.\n")
    elif web_reuse:
    ...
    if budget and (tools or web):
        lines.append(f"- 이 단계에서 도구 호출(검색·페이지 조회·명령)은 {budget}회 이내로 하십시오. 핵심 사실부터 확인하고 사소한 것은 넘기십시오.\n")
    return "".join(lines)
...
COMMON_RULES_TAIL = (
    ...
    "- 대화 기록의 '[프로그램 기록 · ...]' 블록은 프로그램이 기록한 실제 도구 사용(명령·결과·검색·파일 변경)입니다. "
    "상대의 주장이 이 기록과 다르면 기록을 믿으십시오.\n"
)
COMMON_RULES = COMMON_RULES_HEAD + TOOL_RULE_OFF + COMMON_RULES_TAIL  # 도구 꺼진 기본 규칙 (하위 호환)
